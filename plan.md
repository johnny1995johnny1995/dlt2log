...
- [ ] 專案初始化（Cargo.toml 與依賴設定）
- [ ] 實作 DLT 檔案讀取器（支援 Storage Header 自動偵測）
- [ ] 實作 DLT v1/v2 訊息解析（處理 HTYP2 與變動長度 ID）
- [ ] 實作目標文字格式化（輸出絕對時間微秒戳記）
- [ ] 命令列介面（支援輸入、自動預設輸出路徑與詳細模式）

### Phase 2: 進階功能 🚀

- [ ] 支援 FIBEX 檔案（解析 XML 並還原 non-verbose 訊息）
